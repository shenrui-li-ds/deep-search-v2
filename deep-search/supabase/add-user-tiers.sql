...
-- Allows VIP users to have higher monthly free credits
...
-- Usage: SELECT set_user_tier('user@example.com', 'vip');
...
BEGIN
  -- Validate tier
  IF p_tier NOT IN ('free', 'vip', 'admin') THEN
    RETURN json_build_object('success', FALSE, 'error', 'Invalid tier. Must be: free, vip, or admin');
  END IF;
